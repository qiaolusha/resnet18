...
# 1.2.4 创建数据加载器，从已经下载好的数据集中，每次随机采样，取五张，加载数据的线程数量为0
...
# region 二.确定网络，定义nn.Sequential模块，包括两个卷积层和两个归一层
...
class ResNet18(nn.Module):
    def __init__(self, BasicBlock, num_classes=1) -> None:
        # ##定义模型的构造函数。
        # 它接受两个参数，一个是BasicBlock类（表示残差块），另一个是num_classes（表示分类类别数，默认值为1）
...
for epoch in range(1, n_epochs + 1):  # epoch从1到9重复执行下列代码8次
    ...
    for data, target in train_loader:
        ...
        # 将当前epoch的损失累加到train_loss变量中，data.size(0)得到批量大小
    ...
    for data, target in valid_loader:
        ...
        output = model(data)
        # 这行代码使用输入的数据（data）来运行模型，并将输出存储在变量output中。
...
# 4.2开始测试
model.eval() 
# 模型不会进行反向传播（即不会更新权重），这是为了确保测试的准确性
state_dict = torch.load('res18.pt')
model.load_state_dict(state_dict)
# 加载模型权重，从名为'res18.pt'的文件中加载状态字典，并将其加载到模型中。
...
for data, target in test_loader:  
    ...
    # 通过此行代码求得索引值，这里预测得到的小于0.5的shiwei0，大于0.5的视为1。以此分类，并将结果存储在变量pred中。
